...
export const fetchScholarsForDepartmentUser = async (faculty, department) => {
  try {
    ...
    if (expectedStatus && departmentCode !== 'UNKNOWN') {
      ...
      console.warn('⚠️ Could not determine Department Code. Returning empty list.');
    ...
    if (error) {
      console.error('❌ Error fetching scholars for department user:', error);
      return { data: null, error };
    }

    console.log(`✅ Found ${data ? data.length : 0} scholars for ${departmentCode}`);
...
export const fetchScholarsForDepartmentUserFlexible = async (faculty, department) => {
  try {
    ...
    if (error) {
      console.error('❌ Error fetching scholars (flexible):', error);
      return { data: null, error };
    }

    return { data, error: null };
  } catch (err) {
    console.error('❌ Exception in fetchScholarsForDepartmentUserFlexible:', err);
    return { data: null, error: err };
  }
};
