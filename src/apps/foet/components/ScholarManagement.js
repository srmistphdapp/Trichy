...
const ScholarManagement = () => {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const allIds = filteredScholars.map(s => s.id);
  ...
  const isAllSelected = filteredScholars.length > 0 && selectedScholarIds.length === filteredScholars.length;
              ...
              filteredScholars.map((scholar, index) => {
                ...
                return (
                  <tr key={scholar.id}>
                    ...
                    <td className="type-column">{scholar.program_type || extractProgramType(scholar.program) || '-'}</td>
