...
const VerifiedScholars = ({ onFullscreenChange, onModalStateChange }) => {
  ...
  const [transferFaculty, setTransferFaculty] = useState('');
  ...
  // Handle transfer scholar - show modal with faculty/department selection
  ...
  // Load departments when transfer faculty changes
  const handleTransferFacultyChange = async (facultyName) => {
    setTransferFaculty(facultyName);
    setTransferDepartment('');
    setTransferDepartments([]);
    if (facultyName) {
      const { data, error } = await fetchDepartmentsByFaculty(facultyName);
      if (!error && data) {
        setTransferDepartments(data);
      }
    }
  };

  // Confirm transfer scholar with new faculty/department
  const confirmTransfer = async () => {
    if (!transferringScholar || !transferFaculty || !transferDepartment) {
      showMessage('Please select both faculty and department to transfer', 'error');
      return;
    }

    setTransferLoading(true);
    try {
      // Derive institution name from faculty (same logic as Edit's handleInputChange)
      let institutionName = transferFaculty;
      if (transferFaculty.includes('Engineering')) {
        institutionName = 'Engineering And Technology';
      } else if (transferFaculty.includes('Science')) {
        institutionName = 'Science And Humanities';
      } else if (transferFaculty.includes('Management')) {
        institutionName = 'Management';
      } else if (transferFaculty.includes('Medical') || transferFaculty.includes('Health')) {
        institutionName = 'Medical And Health Sciences';
      }

      // Build program from department (same pattern as Edit: "Ph.d. - Department Name")
      const newProgram = `Ph.d. - ${transferDepartment}`;

      // Build forwarded status based on new faculty
      let forwardedStatus = 'Forwarded';
      const lowerFaculty = transferFaculty.toLowerCase();
      ...
      if (lowerFaculty.includes('engineering')) {
        ...
        const cleanName = transferFaculty
          .replace('Faculty of ', '')
          .replace(' of ', '')
          .replace('&', 'and')
          .trim();
      ...
      // Use updateScholar service — same as Edit functionality
      const updateData = {
        faculty: transferFaculty,
      ...
      showMessage(
        `✅ ${scholarName} has been successfully transferred to ${transferFaculty} — ${transferDepartment}!`,
        'success'
      );
  ...
  useEffect(() => {
    ...
    showTransferModal, showDownloadModal, showBulkForwardModal, showBulkDeleteModal, onModalStateChange]);
  ...
  return (
    <div className={isFullscreen ? 'fullscreen-mode' : ''}>
      ...
      {/* Transfer Scholar Modal — Faculty & Department Selection */}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-6 rounded-t-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-green-100 text-sm">Select new faculty & department</p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-800">{transferringScholar.select_institution || transferringScholar.faculty || '-'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 mb-1">Department</p>
                    <p className="text-sm font-medium text-gray-800">{transferringScholar.dept_name || transferringScholar.department || '-'}</p>
                  </div>
                </div>
              </div>

              {/* Transfer To — Faculty & Department Dropdowns */}
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-2">
                <p className="text-sm font-semibold text-green-700 mb-3">🔄 Transfer To</p>
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Select Faculty *</label>
                    <select
                      value={transferFaculty}
                      onChange={(e) => handleTransferFacultyChange(e.target.value)}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-transparent bg-white"
                      disabled={!transferFaculty}
                    >
                      <option value="">{transferFaculty ? '-- Select Department --' : '-- Select Faculty First --'}</option>
                      {transferDepartments.map((dept, idx) => (
                        <option key={idx} value={dept.department_name}>{dept.department_name}</option>
                      ))}
                    </select>
                  </div>
                </div>
            ...
            <div className="bg-gray-50 px-6 py-4 rounded-b-xl flex justify-end gap-3">
              ...
              <button
                ...
                disabled={!transferFaculty || !transferDepartment || transferLoading}
                className={`px-6 py-2 rounded-lg transition-colors font-medium flex items-center gap-2 ${!transferFaculty || !transferDepartment || transferLoading
