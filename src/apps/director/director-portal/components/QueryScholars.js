...
const QueryScholars = ({ onFullscreenChange, onModalStateChange }) => {
  ...
  const [showQueryResolvedModal, setShowQueryResolvedModal] = useState(false);
  const [queryResolvedForAll, setQueryResolvedForAll] = useState(false);
  ...
  // Handle forward scholar - show query resolved confirmation first
  const handleForward = (scholar) => {
    setForwardingScholar(scholar);
    setQueryResolvedForAll(false);
    setShowQueryResolvedModal(true);
  };

  // Handle query resolved confirmation
  const handleQueryResolvedConfirmation = (isResolved) => {
    if (isResolved) {
      // Query is resolved, proceed to forward confirmation
      setShowQueryResolvedModal(false);
      if (queryResolvedForAll) {
        setShowForwardAllModal(true);
      } else {
        setShowForwardModal(true);
      }
    } else {
      // Query not resolved, cancel the operation
      setShowQueryResolvedModal(false);
      setForwardingScholar(null);
      setQueryResolvedForAll(false);
      showMessage('Forward cancelled. Please resolve the query first.', 'info');
    }
  };
  ...
  const getFacultyForwardText = (facultyName) => {
    if (!facultyName) return 'Forward to Faculty';
    
    // Map faculty names to forward text
    if (facultyName.toLowerCase().includes('engineering')) return 'Forward to Engineering';
    if (facultyName.toLowerCase().includes('science')) return 'Forward to Science';
    if (facultyName.toLowerCase().includes('management')) return 'Forward to Management';
    if (facultyName.toLowerCase().includes('medical') || facultyName.toLowerCase().includes('health')) return 'Forward to Medical';
    
    // Default: extract first word or use full name
    const firstWord = facultyName.split(' ')[0];
    return `Forward to ${firstWord}`;
  };
  ...
  const confirmForward = async () => {
    if (forwardingScholar) {
      try {
        const facultyForwardText = getFacultyForwardText(forwardingScholar.faculty);
        
        // Update in Supabase
        const { error } = await updateScholar(forwardingScholar.id, {
          query_resolved: 'Query Resolved',
          query_faculty: facultyForwardText,
          status: 'Forwarded',
  ...
  const handleForwardAll = () => {
    ...
    const eligibleScholars = getFilteredScholars().filter(s => {
      // Check if scholar has required fields filled
      const hasName = s.registered_name || s.name;
      const hasEmail = s.email || s.email_id;
      const hasMobile = s.mobile_number || s.mobile;
      const hasFaculty = s.faculty;
      const hasProgram = s.program;
      
      return hasName && hasEmail && hasMobile && hasFaculty && hasProgram;
    });
  ...
  const confirmForwardAll = async () => {
    ...
    try {
      ...
      const updatePromises = eligibleScholars.map(scholar => {
        const facultyForwardText = getFacultyForwardText(scholar.faculty);
      ...
      setBackToDirectorScholars(prev => prev.map(s => {
        ...
        if (isEligible) {
          const facultyForwardText = getFacultyForwardText(s.faculty);
          return {
            ...s,
            query_resolved: 'Query Resolved',
            query_faculty: facultyForwardText,
  ...
  return (
    <div className={isFullscreen ? 'fullscreen-mode' : ''}>
      ...
      {/* Query Resolved Confirmation Modal */}
      {showQueryResolvedModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style={{ paddingTop: '60px' }}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 transform transition-all duration-300" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-gray-900">Query Resolution</h3>
                <button onClick={() => { setShowQueryResolvedModal(false); setForwardingScholar(null); setQueryResolvedForAll(false); }} className="text-gray-400 hover:text-gray-600 transition-colors">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
              ...
              <div className="text-center mb-6">
                ...
                <h4 className="text-lg font-medium text-gray-900 mb-2">Is the Query Resolved?</h4>
                <p className="text-gray-600 mb-2">
                  {queryResolvedForAll 
                    ? `Have all queries been resolved for the scholars you want to forward?`
                    : `Has the query been resolved for ${forwardingScholar?.registered_name || forwardingScholar?.name || 'this scholar'}?`
                  }
                </p>
                <p className="text-sm text-blue-600">Selecting "Yes" will proceed to forward the scholar(s).</p>
              </div>

              <div className="flex gap-3">
                <button
                  onClick={() => handleQueryResolvedConfirmation(false)}
                  className="flex-1 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors font-medium"
                >
                  No
                </button>
                <button
                  onClick={() => handleQueryResolvedConfirmation(true)}
